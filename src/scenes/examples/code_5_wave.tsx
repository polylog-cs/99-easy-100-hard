...
#######################################################`;
  ...
  const gridReferences: Reference<Rect>[][] = grid.map((line) =>
    line.map((_) => createRef<Rect>()),
  );
  ...
  for (let i = 0; i < gridReferences.length; i++) {
    for (let j = 0; j < gridReferences[i].length; j++) {
      const ref = gridReferences[i][j];
      ref().opacity(1);
    }
  }
  ...
  let colors = chroma
    .scale(['#ef476f', '#ffd166', '#06d6a0', '#118ab2'])
    .colors(maxDistance + 1);

  yield* all(
    ...gridReferences.flat().map((ref, i) => {
      let x = i % cols;
      let y = Math.floor(i / cols);

      let distance = distances[y][x];
      let color = grid[y][x] == '#' ? Solarized.base3 : colors[distance];
